...
Mainflux uses the `access_token` provided by Google only to fetch user information which includes user id, name, given name, family name, picture and locale. The `access_token` is not stored in the database and it's not used for any other purpose. The `id_token` is stored in the HTTP only cookie and it's used to authenticate the user in the subsequent requests. The `id_token` is not stored in the database and it's not used for any other purpose.
